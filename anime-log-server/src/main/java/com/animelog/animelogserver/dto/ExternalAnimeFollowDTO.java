...
@Data
@Schema(description = "外部番剧追番请求参数")
public class ExternalAnimeFollowDTO {
    @Schema(description = "数据源提供方", example = "bangumi")
    @NotBlank(message = "数据源不能为空")
    private String sourceProvider;

    @Schema(description = "外部条目ID", example = "1")
    @NotNull(message = "外部条目ID不能为空")
    private Long sourceSubjectId;

    @Schema(description = "追番状态 0想看 1在看 2看完 3搁置 4弃番", example = "0")
    @NotNull(message = "追番状态不能为空")
    @Min(value = 0, message = "追番状态不正确")
    @Max(value = 4, message = "追番状态不正确")
    private Integer watchStatus;

    @Schema(description = "当前看到第几集", example = "0")
    ...
    @Schema(description = "是否收藏 0否 1是", example = "0")
    @Min(value = 0, message = "收藏状态不正确")
    @Max(value = 1, message = "收藏状态不正确")
    private Integer isFavorite;

    @Schema(description = "开始追番日期", example = "2026-04-08")
